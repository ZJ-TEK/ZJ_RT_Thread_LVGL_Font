...
typedef struct
{
    ...
    uint16_t                    last_unicode_cache;      // Last character in the cache
...
/*******************************************************************************************************/
// Private variables
static font_lib_info_t  font_lib_info = {NULL, false};
static font_user_data_t font_user_data[e_FontTypeMax];
...
#if !CMAP_HASH_ENABLE
int binary_method_search(const cmap_sparse_table_t* p_cmap_table, uint16_t cmap_size, uint16_t unicode)
{
    uint16_t mid = 0;
    uint16_t min = 0;
...
static cmap_sparse_table_t* __get_cmap_by_unicode(const public_font_t* pFont, uint32_t unicode)
{
    ...
    if(p_font_info->p_cmap_table == NULL)
    {
        ...
        if(p_font_info->cmap_continued == 1)
        {
            ...
            {
                if(sizeof(cmap_continous_table_t) ==
                  (*p_font_user_data->p_get_data_func)(offset, (uint8_t*)&cmap_info, sizeof(cmap_info)))
                {
...
#if(EXT_FONT_DSC_CACHE_ENABLE)
    FONTTYPE_ENUM FontTypeTemp;
    FontTypeTemp = app_font_get_type(pFont);
...
#if(EXT_FONT_DSC_CACHE_ENABLE)
    FONTTYPE_ENUM Enum_FontType = app_font_get_type(pFont);
    if (Enum_FontType != e_FontTypeMax)
    {
...
int ext_font_create(FONTTYPE_ENUM font_type, public_font_t *pFont, const font_type_info_t *p_font_info, pfunc_get_data p_get_data_func)
{
    font_user_data_t* p_font_user_data = NULL;
    ...
    // Font lib init
    if(!__font_lib_init(&font_lib_info, FONT_LIB_VERSION, 0,  p_get_data_func))
    {
        goto ERROR;
    }
