...
extern int ext_font_create(FONTTYPE_ENUM font_type, public_font_t *p_Font, const font_type_info_t *p_font_info, pfunc_get_data p_get_data_func);
extern void ext_font_delete(public_font_t **p_font);

#if CMAP_HASH_ENABLE
extern int hash_unicode_get_index(const cmap_sparse_table_t* p_cmap_hash_table, uint16_t cmap_size, uint16_t unicode);
#else
extern int binary_method_search(const cmap_sparse_table_t* cmap_table, uint16_t cmap_size, uint16_t unicode);
#endif
