...
//    }
    ...
    rect_info.x = area->x1;
    rect_info.y = area->y1;
    rect_info.width = area->x2 - area->x1 + 1;
    rect_info.height = area->y2 - area->y1 + 1;
//    rt_device_control(device, RTGRAPHIC_CTRL_RECT_UPDATE, &rect_info);
    lcd_ie3130b_put_gfx(rect_info.x, rect_info.y, rect_info.width, rect_info.height, (uint8_t *)color_p);
...
rt_err_t lvgl_rtt_init(const char *name) 
{
    ...
    lv_disp_draw_buf_init(&disp_buf, info.framebuffer, NULL, MY_DISP_VER_RES * MY_DISP_HOR_RES);   /*Initialize the display buffer*/
