...
static void key_handle(void *args)
{
   ...
   else if(pin == ZJ_KEY2 && polarity == NRF_GPIOTE_POLARITY_HITOLO)
   {

   }
   else if(pin == ZJ_KEY3 && polarity == NRF_GPIOTE_POLARITY_HITOLO)
   {

   }
   else if(pin == ZJ_KEY4 && polarity == NRF_GPIOTE_POLARITY_HITOLO)
   {
...
static int led_key_init(void)
{
    ...
    rt_pin_attach_irq(ZJ_KEY2, RT_GPIO_MODE_SET(NRF_GPIOTE_POLARITY_HITOLO,NRF_GPIO_PIN_SENSE_LOW),key_handle, RT_NULL);
    rt_pin_irq_enable(ZJ_KEY2, PIN_IRQ_ENABLE);
    rt_pin_attach_irq(ZJ_KEY3, RT_GPIO_MODE_SET(NRF_GPIOTE_POLARITY_HITOLO,NRF_GPIO_PIN_SENSE_LOW),key_handle, RT_NULL);
    rt_pin_irq_enable(ZJ_KEY3, PIN_IRQ_ENABLE);
    rt_pin_attach_irq(ZJ_KEY4, RT_GPIO_MODE_SET(NRF_GPIOTE_POLARITY_HITOLO,NRF_GPIO_PIN_SENSE_LOW),key_handle, RT_NULL);
    rt_pin_irq_enable(ZJ_KEY4, PIN_IRQ_ENABLE);
    return 0;
}
...
void led_timer_init(void)
{
    ...
    rt_kprintf("ZJ-SDK:RT-Thread֨ʱǷ՚ZJӜďւߪעѥʏքӦԃ\n");   
    rt_kprintf("led_timeout1_handler 4s۳ֻܡִѐһՎߍעк\n");
    rt_kprintf("led_timeout2_handler ܡÿٴ2sѭ۷ִѐ\n");
